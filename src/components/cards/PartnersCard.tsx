...
const PartnersCard: React.FC<CardProps> = ({ image }) => {
  ...
  return (
    <motion.div
      className="relative overflow-hidden h-[150px] min-w-max rounded-xl flex justify-center items-center p-5"
    //   key={image}
    ...
    >
      ...
      {image && <Image src={image} alt="image" width={250} height={250} className="w-full max-h-20 h-full object-fit" />}
    </motion.div>
  );
};
